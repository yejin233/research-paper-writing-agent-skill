...
## Monitoring (Windows/Codex Pattern)

### Monitoring Prompt Template

For each experiment batch, use current-session polling when Codex remains open. For unattended checks, use Windows Task Scheduler or a user-managed terminal:

```
Check the status of the [EXPERIMENT_NAME] experiment:

1. Process check: Get-Process python -ErrorAction SilentlyContinue
2. Log check: Get-Content [LOG_FILE] -Tail 30
3. Results check: Get-ChildItem [RESULT_DIR]\eval\ (or appropriate result location)
